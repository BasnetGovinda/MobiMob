	...
	var xhr = Ti.Network.createHTTPClient({
		onload : function(e) {
			...
			if (headcount.rowCount != 0) {
				...
				db1.execute("UPDATE  Inspection set synDate='" + sydate1 + "',Updatedate='" + update + "', VIN='" + VIN.value + "', Series='" + Category.value + "',Model='" + Model.value + "'    where EntryHeaderID =" + v);
			}
...
function imageLoader(currentimg) {
	...
	if (currentimg < imgbolb.length) {
		var imgdb = Ti.Database.open('mydatalist27');
		var imagequery = "Select * from tbl_temp where imageName='" + self.picview[contarray[currentimg]].imagename + "'";
		var imgdata = imgdb.execute(imagequery);
		console.log(imgdata.fieldByName("imageBlob"));

		self.picview[contarray[currentimg]].image = Ti.Utils.base64decode(imgdata.fieldByName("imageBlob"));
